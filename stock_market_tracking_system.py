...
from weekly_data_sources import (
    ...
    _fetch_close_series,
    _fetch_twse_index_data,
    _fetch_twse_stock_data,
    _find_exact_field,
    _find_field,
    ...
    fetch_market_institutional_value_day,
    fetch_market_institutional_value_week,
    fetch_twse_closed_dates,
    fetch_weekly_institutional,
    is_twse_trading_day,
...
def fetch_weekly_market_inputs(cfg: dict, target_dt: datetime) -> dict:
...
def analyze_weekly_watchlist(cfg: dict, run: dict, market_inputs: dict) -> list:
    results = []
    for stock in cfg["watchlist"]:
        ...
        try:
            ...
            df = fetch_data(ticker, cfg["lookback_days"], run["target_date"])
            ...
            market_inst_value_week = market_inputs["market_inst_value_week"]
...
def main():
    cfg = load_config()
    run = prepare_weekly_run(cfg)
    print(
        f"[{run['now_tw'].strftime('%Y-%m-%d %H:%M')}] 週報目標交易日={run['expected_date']}，"
    ...
    validate_complete_report_results(results, cfg["watchlist"], run["expected_date"])
    print(
        f"✅ 週報完整性檢查通過：{len(results)}/{len(cfg['watchlist'])} 檔，"
        f"資料日={run['expected_date']}"
    )
